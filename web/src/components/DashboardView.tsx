...
import {
  CodexSessionShelf,
  type SessionPromotionDraft,
  type SessionShelfSnapshot,
} from "./CodexSessionShelf";
...
interface DashboardViewProps {
  ...
  onOpenCodexSession: (threadId: string) => void;
  onPromoteSession: (
    session: SessionShelfSnapshot["sessions"][number],
...
export function DashboardView({
  ...
  const [sessionShelf, setSessionShelf] = useState<SessionShelfSnapshot>({
    sessions: [],
    watches: [],
  });
  ...
  const trackedTaskThreads = new Set(tasks.flatMap((task) => [
    task.threadId,
    ...task.conversationRefs.map((ref) => ref.threadId),
  ].filter((threadId): threadId is string => Boolean(threadId))));
  const nativeRunningSessions = sessionShelf.sessions.filter((session) => (
    session.running && !trackedTaskThreads.has(session.threadId)
  ));
  ...
  const attentionItems = activeTasks
    ...
    .slice(0, 5);
  const attentionSessions = sessionShelf.watches
    .filter((watch) => watch.needsReview)
    .map((watch) => sessionShelf.sessions.find((session) => session.threadId === watch.threadId))
      ...
      <div className="dashboard-content">
        ...
        <div className="dashboard-grid">
          ...
          <section className="dashboard-panel dashboard-primary-panel dashboard-attention-panel">
            ...
            <div className="dashboard-task-list">
              ...
              {attentionSessions.map((session) => (
                <button
                  type="button"
                  className="dashboard-attention-row dashboard-session-attention-row"
                  onClick={() => onOpenCodexSession(session.threadId)}
                  key={`session:${session.threadId}`}
                >
                  <span className="dashboard-attention-mark is-session" aria-hidden="true"><i /></span>
                  <strong>{session.title}</strong>
                  <small>待确认</small>
                </button>
              ))}
              {!attentionItems.length && !attentionSessions.length && (
          ...
          <section className="dashboard-panel dashboard-primary-panel dashboard-running-panel">
            ...
            <div className="dashboard-task-list">
              {runningTasks.length || nativeRunningSessions.length ? <>
                ...
                {nativeRunningSessions.map((session) => (
                  <article
                    className="dashboard-running-card dashboard-native-running-card"
                    key={`session:${session.threadId}`}
                    onClick={() => onOpenCodexSession(session.threadId)}
                  >
                    <button
                      ...
                      onClick={(event) => {
                        event.stopPropagation();
                        onOpenCodexSession(session.threadId);
                      }}
                    >
                      <small>本机 Codex · {session.cwd?.split("/").filter(Boolean).at(-1) ?? "未知目录"}</small>
                      <strong>{session.title}</strong>
                    </button>
                    <div className="dashboard-running-footer">
                      <span className="task-processing is-running">
                      ...
                      <span className="dashboard-native-session-label">未提升为议题</span>
                    </div>
                  </article>
                ))}
              </> : (
          ...
          <CodexSessionShelf
            ...
            onSnapshotChange={setSessionShelf}
