...
import type { SessionPromotionDraft } from "./components/CodexSessionShelf";
...
type BoardView = "dashboard" | "issues" | "list" | "gantt" | "outcomes" | "workflow";
...
const OutcomeGraphView = lazy(() => import("./components/OutcomeGraphView").then((module) => ({
  default: module.OutcomeGraphView,
})));
...
function readProjectBoardView(projectId: string): BoardView {
  ...
  return view === "dashboard" || view === "list" || view === "gantt" || view === "outcomes" || view === "issues"
    ? view
    : "issues";
}
...
export function App() {
  ...
  async function promoteCodexSession(
    session: { threadId: string; cwd: string | null },
    draft: SessionPromotionDraft,
  ): Promise<Task> {
    if (!selectedProjectId) throw new Error(text("请先选择一个项目。", "Select a project first."));
    const normalizedThreadId = normalizeCodexThreadId(session.threadId);
    const existing = tasksRef.current.find((task) => (
      normalizeCodexThreadId(task.threadId) === normalizedThreadId
      || task.conversationRefs.some((reference) => (
        normalizeCodexThreadId(reference.threadId) === normalizedThreadId
      ))
    ));
    ...
    const description = [
      "来源：Codex session",
      `工作目录：${session.cwd ?? "未知工作目录"}`,
      `原始会话：codex://threads/${session.threadId}`,
      "",
      "为什么提升为议题：",
      draft.note || "（未填写）",
    ].join("\n");
    const task = await createTaskRequest(selectedProjectId, {
      title: draft.title,
      description,
      status: draft.status,
      priority: "none",
    ...
    setAnnouncement(text(
      `${task.identifier} 已从 Codex 会话提升为议题。`,
      `${task.identifier} was promoted from a Codex session.`,
    ));
    return task;
  }
      ...
      <main className="workspace">
        ...
        {selectedProjectId && !detailTask && <div className="board-toolbar">
          <div className="view-tabs" aria-label={text("看板视图", "Board views")}>
            ...
            <button
              className={`view-tab${boardView === "outcomes" ? " active" : ""}`}
              type="button"
              aria-pressed={boardView === "outcomes"}
              onClick={() => selectBoardView("outcomes")}
            >
              {text("结果血缘", "Outcome lineage")}
            </button>
        ...
        {detailTask && selectedProject ? (
          ...
          <DashboardView
            ...
            currentUser={currentUser}
            projectName={selectedProject?.id === GLOBAL_PROJECT_ID
              ? text("全局", "Global")
            ...
            onOpenCodexSession={openLegacyLocalThread}
            onPromoteSession={promoteCodexSession}
          />
        ) : boardView === "list" ? (
          <IssueListView
          ...
          <Suspense fallback={<div className="workflow-board-loading">{text("正在打开结果血缘…", "Opening outcome lineage…")}</div>}>
            <OutcomeGraphView />
          </Suspense>
        ) : boardView === "workflow" ? (
