...
export async function listCodexSessions(
  limit = 40,
  signal?: AbortSignal,
): Promise<CodexSessionCatalogEntry[]> {
  const data = await request<{ sessions: CodexSessionCatalogEntry[] }>(
    `/api/local/codex-sessions?limit=${limit}`,
    { signal },
  );
  return data.sessions;
}

export async function getSessionOutcomeGraphDraft(signal?: AbortSignal): Promise<OutcomeGraphDraft> {
  const data = await request<{ draft: OutcomeGraphDraft }>("/api/local/session-outcome-graph", { signal });
  return data.draft;
}
