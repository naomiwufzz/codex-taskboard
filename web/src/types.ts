...
export type OutcomeStatus = "done" | "partial" | "blocked";
export type OutcomeEvidenceClass =
  | "local_run_summary"
  | "workflow_design_and_local_output"
  | "governance_rule"
  | "static_sql_artifact"
  | "local_data_finding"
  | "planned_design"
...
export type OutcomeRelationType = "continues" | "informs" | "constrains" | "supports";

export interface OutcomeWorkContext {
  trigger: string;
  goal: string;
  finishDefinition: string;
  actualStopPoint: string;
  notToBeMisreadAs: string;
}

export interface OutcomeLineageFrame {
  title: string;
  summary: string;
...
export interface OutcomeResultCard {
  id: string;
  title: string;
  type: string;
  ...
  workContext: OutcomeWorkContext;
  outcome: string;
  evidencePointers: string[];
...
export interface OutcomeParentWork {
  id: string;
  title: string;
  status: "candidate" | "confirmed";
  reason: string;
  upstreamContext?: OutcomeLineageFrame;
  downstreamGoal?: OutcomeLineageFrame;
}

export interface OutcomeLineageRelation {
  id: string;
  fromResultId?: string;
  toResultId?: string;
  fromUnitId?: string;
  toUnitId?: string;
  relation: OutcomeRelationType;
  relationScope: "method_continuation" | "unit_reasoning";
  status: "evidence_backed" | "candidate";
  confidence: "high" | "medium" | "low";
  rationale: string;
  evidencePointers?: string[];
  requiresUserConfirmation?: boolean;
  defaultVisibility: "primary" | "on_demand";
}

export interface OutcomeLineageDisplayPolicy {
  defaultView: "mainline_only";
  mainlineRule: string;
  detailRevealRule: string;
  candidateRule: string;
  maxExpandedUnitEdges: number;
  nonGoal: string;
}

export interface OutcomeGraphDraft {
  schemaVersion: number;
  purpose: string;
  status: string;
  createdAt: string;
  parentWorkCandidates: OutcomeParentWork[];
  resultCards: OutcomeResultCard[];
  lineageRelations: OutcomeLineageRelation[];
  lineageDisplayPolicy: OutcomeLineageDisplayPolicy;
}
...
export interface CodexSessionCatalogEntry {
  threadId: string;
  cwd: string | null;
  startedAt: string;
  updatedAt: string;
