...
async function main() {
  ...
  const requestTaskboardOpen = async (expectedSourceHash) => {
    ...
    try {
      ...
      const frameLoaded = status.frameUrl
        ? await waitForFrame(connection, status.frameUrl, 15_000)
        : false;
      if (!status.pageVisible || !status.frameReady || !frameLoaded) {
        throw new Error("Taskboard frame did not report ready after opening");
      }
      await connection.send("Page.bringToFront");
      openedRequestGeneration = Math.max(openedRequestGeneration, generation);
      return { status, frameLoaded };
    } catch (error) {
  ...
  try {
    ...
    if (stopping) return;
    let firstOpenResult = null;
    if (hasOpenPending() && injectedTargets.size > 0) {
      firstOpenResult = await requestTaskboardOpen(sourceHash);
      if (firstOpenResult?.status && firstResults.length > 0) {
        firstResults[0] = {
          ...firstResults[0],
          ...firstOpenResult.status,
          frameLoaded: firstOpenResult.frameLoaded,
        };
      }
    }
    ...
    while (!stopping) {
      ...
      try {
        ...
        let openResult = null;
        if (hasOpenPending() && injectedTargets.size > 0) {
          openResult = await requestTaskboardOpen(sourceHash);
          if (openResult?.status && results.length > 0) {
            results[0] = {
              ...results[0],
              ...openResult.status,
              frameLoaded: openResult.frameLoaded,
            };
          }
        }
        if (results.length > 0) {
