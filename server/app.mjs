...
const CODEX_SESSION_CATALOG_CACHE_MS = 8_000;
const CODEX_SESSION_CATALOG_LIMIT = 100;
const CODEX_SESSION_HEAD_BYTES = 768 * 1024;
...
function textFromSessionMessage(value) {
...
function sessionBusinessRequest(value) {
  if (!value || /^\s*<(?:environment_context|skill)>/i.test(value)) return "";
  const request = value.match(/## My request(?: for Codex)?:\s*([\s\S]+)/i);
  return request ? request[1] : value;
}
...
export function createTaskboardServer(options = {}) {
  ...
  let codexSessionCatalogCache = null;

  async function readJsonlSlice(filename, offset, length) {
  ...
  function parseJsonlRecords(value, discardFirstLine = false) {
    const lines = value.split("\n");
    if (discardFirstLine) lines.shift();
    const records = [];
  ...
  function sessionUserMessage(record) {
    const payload = record?.payload;
    if (record?.type !== "response_item" || payload?.type !== "message" || payload.role !== "user") {
      return "";
    }
    return textFromSessionMessage(payload.content);
  }

  async function readCodexSessionCatalogEntry(filename, fileStats) {
    const headLength = Math.min(fileStats.size, CODEX_SESSION_HEAD_BYTES);
    const headRecords = parseJsonlRecords(await readJsonlSlice(filename, 0, headLength));
    const meta = headRecords.find((record) => record?.type === "session_meta")?.payload ?? {};
    ...
    for (const record of headRecords) {
      const userMessage = sessionUserMessage(record);
      if (/^\s*<codex_delegation>/i.test(userMessage)) {
        title = "Codex 子会话";
        break;
      }
      title = compactSessionTitle(sessionBusinessRequest(userMessage));
      if (title) break;
    }

    const tailLength = Math.min(fileStats.size, CODEX_SESSION_TAIL_BYTES);
    const tailRecords = parseJsonlRecords(
      await readJsonlSlice(filename, fileStats.size - tailLength, tailLength),
      tailLength < fileStats.size,
    );
    ...
    // A long active desktop turn can push task_started outside the small catalog tail.
    const liveState = fileStats.mtimeMs >= Date.now() - CODEX_SESSION_CATALOG_CACHE_MS * 2
      ? await readCodexSessionState(threadId).catch(() => null)
      : null;

    return {
      threadId,
      cwd: typeof meta.cwd === "string" ? meta.cwd : null,
      startedAt: typeof meta.timestamp === "string" ? meta.timestamp : fileStats.birthtime.toISOString(),
      updatedAt: fileStats.mtime.toISOString(),
      title: title || "未命名 Codex 会话",
      running: liveState?.running ?? runningTurnId !== null,
    };
  }

  async function listCodexSessionCatalog(limit) {
    const cached = codexSessionCatalogCache;
    if (cached && Date.now() - cached.createdAt < CODEX_SESSION_CATALOG_CACHE_MS) {
    ...
    while (directories.length > 0) {
      ...
      for (const entry of entries) {
        ...
        else if (entry.isFile() && entry.name.endsWith(".jsonl")) filenames.push(entryPath);
      }
    }

    const candidates = (await Promise.all(filenames.map(async (filename) => {
      try {
        return { filename, fileStats: await stat(filename) };
      } catch {
        return null;
      }
    })))
      .filter(Boolean)
      .sort((left, right) => right.fileStats.mtimeMs - left.fileStats.mtimeMs)
      .slice(0, CODEX_SESSION_CATALOG_LIMIT);
    const entries = (await Promise.all(candidates.map(({ filename, fileStats }) => (
      readCodexSessionCatalogEntry(filename, fileStats).catch(() => null)
    ))))
      .filter(Boolean)
      .sort((left, right) => right.updatedAt.localeCompare(left.updatedAt));
    codexSessionCatalogCache = { createdAt: Date.now(), entries };
    ...
    try {
      ...
      if (pathname === "/api/local/codex-sessions") {
        ...
        if ([...url.searchParams.keys()].some((key) => key !== "limit")) {
          throw new ApiError(400, "UNKNOWN_QUERY_PARAMETER", "Only 'limit' is supported");
        }
        const rawLimit = url.searchParams.get("limit");
        const limit = rawLimit === null ? 40 : Number(rawLimit);
        if (!Number.isInteger(limit) || limit < 1 || limit > CODEX_SESSION_CATALOG_LIMIT) {
          throw new ApiError(400, "INVALID_FIELD", "'limit' must be an integer between 1 and 100");
        }
        return sendJson(response, 200, { sessions: await listCodexSessionCatalog(limit) });
      }

      if (pathname === "/api/local/session-outcome-graph") {
        if (request.method !== "GET") return methodNotAllowed(response, ["GET"]);
        assertNoQuery(url.searchParams, "GET /api/local/session-outcome-graph");
        const draftPath = path.join(PROJECT_ROOT, ".data", "session-outcome-graph-phase2-drafts.json");
        let draft;
        try {
          draft = JSON.parse(await readFile(draftPath, "utf8"));
        } catch (error) {
          if (error?.code === "ENOENT") {
            throw new ApiError(404, "OUTCOME_GRAPH_DRAFT_NOT_FOUND", "Outcome graph draft is not available");
          }
          if (error instanceof SyntaxError) {
            throw new ApiError(500, "OUTCOME_GRAPH_DRAFT_INVALID", "Outcome graph draft is invalid JSON");
          }
          throw error;
        }
