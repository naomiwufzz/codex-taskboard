...
function fallbackSummary(tasks) {
  ...
  const highlights = [
    counts.in_progress ? `${counts.in_progress} 个处理中` : "",
    counts.in_review ? `${counts.in_review} 个等你确认` : "",
    counts.blocked ? `${counts.blocked} 个遇到阻碍` : "",
    counts.todo ? `${counts.todo} 个待办` : "",
  ].filter(Boolean);
  ...
  return [
    `当前共有 ${tasks.length} 个已提升议题${highlights.length ? `，${highlights.join("，")}` : ""}。`,
    nextTask ? `下一步可优先推进「${nextTask.title}」。` : "当前没有待推进的议题。",
  ].join("");
}

function emptyProjectSummary() {
  return "当前还没有已提升的议题。可在 Codex 会话池选择 session，明确提升为待办、处理中或等你确认；原始会话会保留为证据指针。";
}
...
export class ProjectSummaryService {
  ...
  get(projectId) {
    ...
    return {
      projectId,
      summary: summary.summary ?? fallbackSummary(tasks),
